...
设置里勾选**「分段使用不同配置」**，就能让架构段与执行段各指向一套配置，并各自覆盖模型名：
...
命令类验收（pytest、构建脚本等）**不会**自动执行——那需要白名单与逐条人工确认，属于后续计划。
...
- 命令执行能力默认关闭，且目前不提供命令执行的沙箱；**命令类验收**（pytest 等）
  因此也不会自动跑，客观验收只覆盖"文件/内容/语法/JSON"这几类可判定条件；
