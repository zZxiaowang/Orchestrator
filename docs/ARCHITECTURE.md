...
| **P3 UI 收敛** | ✅ 侧栏底部只剩「能力中心 / Git / 更新 / 设置」（插件市场与已装插件并进能力中心的插件分页）；设置收敛为**四区**（模型与路由 / 执行与验收 / 上下文与记忆 / 能力与集成），并把此前只能改 `.env` 的上下文预算搬进界面；折叠规则统一到架构 / 计划 / 验证 / 日志；模块与对话补上**错误态 + 重试**（四态齐了） | 完成 |
